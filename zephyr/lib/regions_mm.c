...
struct vmh_heap {
	...
	unsigned out_of_blocks[MAX_MEMORY_ALLOCATORS_COUNT];
	bool logged;
#endif
...
#ifdef CONFIG_SYS_MEM_BLOCKS_RUNTIME_STATS
/**
 * @brief Print stats on heap usage
 *
...
void vmh_log_stats(struct vmh_heap *heap)
{
	...
	LOG_INF("Virtual heap stats per region");
	LOG_INF(" ID    | Total  | Max    | Times run out of blocks");
	...
	for (int idx = 0; idx < MAX_MEMORY_ALLOCATORS_COUNT; idx++) {
		...
		LOG_INF("%7d| %7u| %7u| %7u", idx, block_num, stats.max_allocated_bytes/block_size, heap->out_of_blocks[idx]);
	}
	heap->logged = true;
}
#endif
